...
class Viewfinder {
  ...
  updateAngle(angleDegrees, eje) {
    ...
    this.lineLeft.style.transform = `translate(0, -50%) rotate(${
      180 - angle - eje + 180
    }deg)`;
    this.lineRight.style.transform = `translate(0, -50%) rotate(${
      angle - eje + 180
    }deg)`;
  }
...
class Drone {
  ...
  updatePosition() {
    ...
    this.position.z =
      config.drone.orbitRadius * Math.sin(this.angle * 0.7) + 1000;
    this.position.y =
      config.drone.height +
      Math.sin(this.angle * 2) * config.drone.bobAmount +
      600;
    let angleInRadiansX =
      -Math.atan2(this.position.z, this.position.x) + Math.PI + Math.PI / 2;
...
class JoystickControls {
  ...
  update(delta) {
    ...
    if (!followDrone) {
      const lookX = this.applyDeadZone(
        gamepad.axes[0],
        config.gamepad.deadZone
      );
      const lookY = this.applyDeadZone(
        gamepad.axes[1],
        config.gamepad.deadZone
      );
...
function init() {
  ...
  new RGBELoader().load("environmentMap/road4k.hdr", (texture) => {
    texture.mapping = THREE.EquirectangularReflectionMapping;
    scene.background = texture;
    scene.environment = texture;
  });

  new GLTFLoader().load("/models/mavic.glb", (gltf) => {
  ...
  new GLTFLoader().load("/models/MSP_VAMTAC.glb", (gltf) => {
    const model = gltf.scene;
